...
4. Біля кнопки **Compile** є стрілка вниз (▼) — клікніть на неї, щоб:
   - **Compiler arguments...** — встановити параметри компілятора (зберігаються в `.fidet.ini`)
   - **Reset compiler arguments** — скинути параметри
5. Біля кнопки **Run** є стрілка вниз (▼) — клікніть на неї для:
   - **Run with custom args...** — запустити скомпільовану програму з власними аргументами
6. Натисніть **Compile** (тільки компіляція) або **Run** (компіляція + запуск).

Параметри компілятора зберігаються в конфіг-файлі `.fidet.ini` у корені проєкту.
